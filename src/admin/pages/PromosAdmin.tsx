...
export default function PromosAdmin() {
  ...
  const [filterType, setFilterType] = useState<'all' | 'daypass' | 'resort'>('all');

  const filtered = filterType === 'all' ? data : data.filter(p => p.type === filterType);

  const openNew = (type: 'daypass' | 'resort') => { setForm({ ...empty, type, order: data.length }); setEditing(null); setShowModal(true); };
    ...
    <div>
      <div className="adm-topbar">
        <div className="admin-page__header" style={{ marginBottom: 0 }}>
          <h1 className="admin-page__title">Day Pass & Resort</h1>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <button className="adm-btn adm-btn--primary" onClick={() => openNew('resort')}>+ Resort</button>
        </div>
      </div>

      {/* Filter tabs */}
      <div style={{ display: 'flex', gap: 8, marginBottom: 20 }}>
        {(['all', 'daypass', 'resort'] as const).map(t => (
          <button key={t} className={`adm-btn ${filterType === t ? 'adm-btn--primary' : 'adm-btn--ghost'} adm-btn--sm`} onClick={() => setFilterType(t)}>
            {t === 'all' ? 'Todas' : t === 'daypass' ? 'Day Pass' : 'Resort'}
          </button>
        ))}
      </div>
            ...
            <div key={item.id} style={{ background: '#111d35', borderRadius: 14, overflow: 'hidden', border: '1px solid rgba(255,255,255,0.06)' }}>
              ...
              <div style={{ padding: 14 }}>
                <span className={`adm-tag ${item.type === 'daypass' ? 'adm-tag--gold' : 'adm-tag--green'}`} style={{ marginBottom: 8, display: 'inline-block' }}>
                  {item.type === 'daypass' ? 'Day Pass' : 'Resort'}
                </span>
      ...
      {showModal && (
        <div className="adm-overlay" onClick={close}><div className="adm-modal" onClick={e => e.stopPropagation()}>
          <h3 className="adm-modal__title">{editing ? 'Editar' : 'Nueva'} Promo — {form.type === 'daypass' ? 'Day Pass' : 'Resort'}</h3>
          ...
          <div className="adm-form-row">
            <div className="adm-field"><label>Tipo</label><select value={form.type} onChange={e => setForm({ ...form, type: e.target.value as 'daypass' | 'resort' })}><option value="daypass">Day Pass</option><option value="resort">Resort</option></select></div>
